...
describe 'requesting a merchant' do
  ...
  describe 'a single merchant' do
    before(:each) do
      ...
      @invoice_2 = create(:invoice, merchant: @merchant_1, status: "shipped", updated_at: '2012-03-21 14:54:09 UTC', customer: @customer_2)
      @invoice_3 = create(:invoice, merchant: @merchant_2, status: "shipped", updated_at: '2012-03-24 14:54:09 UTC')
      @invoice_4 = create(:invoice, merchant: @merchant_3, status: "shipped", updated_at: '2012-03-24 14:54:09 UTC')
      @invoice_5 = create(:invoice, merchant: @merchant_4, status: "shipped", updated_at: '2012-03-24 14:54:09 UTC')
      @invoice_6 = create(:invoice, merchant: @merchant_1, status: "shipped", updated_at: '2012-03-24 14:54:09 UTC', customer: @customer_2)
      ...
      @transaction_4 = create(:transaction, invoice: @invoice_4, result: :success, updated_at: '2012-03-24 14:54:09 UTC')
      @transaction_5 = create(:transaction, invoice: @invoice_5, result: :failed, updated_at: '2012-03-24 14:54:09 UTC')
      @transaction_6 = create(:transaction, invoice: @invoice_6, result: :success, updated_at: '2012-03-24 14:54:09 UTC')
    end
    it 'can return revenue for a single merchant' do
      get "/api/v1/merchants/#{@merchant_2.id}/revenue"

      expect(response).to be_successful
      merchants_data = JSON.parse(response.body)

      ex_rev = number_to_currency(@merchant_2.total_revenue({}) / 100.0)[1..-1]

      expect(merchants_data.length).to eq(1)
      expect(merchants_data["data"]["attributes"]["revenue"]).to eq(ex_rev)
    end
    it 'can return revenue for a single merchant on a date' do
      date = '2012-03-20'
      ...
      expect(merchants_data["data"]["attributes"]["revenue"]).to eq("125.00")
    ...
    it 'can return a favorite customer for a merchant' do
      get "/api/v1/merchants/#{@merchant_1.id}/favorite_customer"

      expect(response).to be_successful
      merchants_data = JSON.parse(response.body)

      expect(merchants_data["data"]["attributes"]["id"]).to eq(@customer_2.id)
    end
